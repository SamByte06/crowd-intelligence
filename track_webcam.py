...
# File used to share the latest AI results with the FastAPI backend.
PROJECT_ROOT = os.path.dirname(os.path.abspath(__file__))
STATE_FILE = os.path.join(PROJECT_ROOT, "crowd_state.json")


def save_crowd_state(state):
    """Save the latest crowd intelligence safely for the backend."""
    ...
    try:
        ...
        print(f"Could not save crowd state: {error}")
...
def update_track(track_id, center):
    ...
    if track.prev_center is not None:
        ...
        dx = center[0] - track.prev_center[0]

        dy = center[1] - track.prev_center[1]
        ...
        if track.movement < MOVEMENT_THRESHOLD:
...
while True:
    ...
    for zone in range(
        ...
        congestion = zone_congestion[zone]
    ...
    # ========================================================
    # SAVE LIVE CROWD STATE FOR BACKEND
    # ========================================================
    ...
    for zone in range(GRID_ROWS * GRID_COLS):

        flow = zone_flow[zone]

        zones_data.append({
            "zone": zone,
            "people": zone_counts[zone],
            "density": get_density(zone_counts[zone]),
            "flow": flow,
            "dominant_direction": max(flow, key=flow.get),
            "trend": zone_trends[zone],
            "congestion": zone_congestion[zone],
            "risk": zone_risk_scores[zone],
            "risk_level": zone_risk_levels[zone]
        })
    ...
    for zone in range(
        ...
        zone_x = zone % GRID_COLS

        zone_y = zone // GRID_COLS
        ...
        risk_score = zone_risk_scores[zone]
